...
const Navbar = () => {
  ...
  // Handle scroll for navbar sticky effect
  useEffect(() => {
    const handleScroll = () => {
  ...
  const NavLink = ({ to, children }) => (
    <Link
      to={to}
      className={`${isActive(to) ? 'text-[#E02B2B]' : 'text-white hover:text-[#E02B2B]'} transition-all duration-300 text-[13px] font-bold uppercase flex items-center relative group tracking-wider`}
      onClick={() => setIsOpen(false)}
    >
      <span className="flex items-center gap-1.5">
        {children} <span className="text-[14px] font-normal opacity-70 group-hover:rotate-90 transition-transform duration-300">+</span>
      </span>
      <span className={`absolute -bottom-1 left-0 w-full h-[2px] bg-[#E02B2B] transform origin-left transition-transform duration-300 ${isActive(to) ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'}`}></span>
    </Link>
  );
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 flex flex-col shadow-2xl">
      ...
      <div
        className={`hidden lg:flex justify-between items-center px-12 bg-[#E02B2B] text-white text-xs transition-all duration-500 overflow-hidden ${isScrolled ? 'max-h-0 py-0 opacity-0' : 'max-h-[40px] py-2.5 opacity-100'}`}
      >
        ...
        <div className="flex items-center gap-4">
          <span className="font-bold tracking-wide mr-2">Follow Us:</span>
          <div className="flex items-center gap-3">
            <Link to="#" className="hover:text-black transition-colors transform hover:scale-110"><Facebook size={14} fill="currentColor" /></Link>
            <Link to="#" className="hover:text-black transition-colors transform hover:scale-110"><Twitter size={14} fill="currentColor" /></Link>
            <Link to="#" className="hover:text-black transition-colors transform hover:scale-110"><Instagram size={14} /></Link>
          </div>
        </div>
      </div>
      ...
      <nav className={`w-full bg-[#111111] transition-all duration-300 ${isScrolled ? 'h-[80px]' : 'h-[95px]'}`}>
        <div className="flex justify-between items-stretch h-full max-w-[1600px] mx-auto">
          ...
          <div className="flex items-center gap-0 h-full">
            ...
            {/* Mobile Menu Button */}
            <button className="lg:hidden text-white hover:text-[#E02B2B] px-6 h-full border-l border-[#333]" onClick={() => setIsOpen(!isOpen)}>
